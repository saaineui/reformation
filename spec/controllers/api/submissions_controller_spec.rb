...
RSpec.describe Api::SubmissionsController, type: :controller do
  ...
  describe 'POST #create' do
    ...
    let(:valid_submission) do 
      { 
        :token => hire_form.user.token, :web_form_id => hire_form.id, :source => 'http://valid.io', 
    ...
    let(:unauthorized) do 
      { 
        :web_form_id => hire_form.id, :source => 'http://unauthorized.io', 
    ...
    let(:bad_token) do 
      { 
        :token => '88888888', :web_form_id => hire_form.id, :source => 'http://bad_token.io', 
    ...
    let(:spare_submission) do 
      { 
        :token => hire_form.user.token, :web_form_id => hire_form.id, :source => 'http://spare.io', 
    ...
    let(:missing_source) do 
      { 
        :token => hire_form.user.token, :web_form_id => hire_form.id, :source => '', 
    ...
    let(:incomplete_submission) do 
      { 
        :token => hire_form.user.token, :web_form_id => hire_form.id, :source => 'http://incomplete.io', 
        "web_form_field_#{name_field.id}".to_sym => ' || 1; pg_sleep(1);' 
      } 
    end
    let(:excess_submission) do 
      { 
        :token => hire_form.user.token, :web_form_id => hire_form.id, :source => 'http://excess.io', 
    ...
    # Responses
    let(:success_json) { { 'notice' => 'Thanks for your message!' } }
    let(:failure_json) { { 'notice' => 'You were missing the following required fields: Email.' } }
    let(:missing_source_json) { { 'notice' => 'Your form was missing a source.' } }
    ...
    context 'with valid data' do
      ...
      context 'without source' do
        it 'responds with 401 error with notice without creating submission' do
          post :create, params: missing_source
      ...
      context 'without token' do
        it 'responds with unauthorized' do
          post :create, params: unauthorized
    ...
    context 'with incomplete data' do
      it 'responds with 401 error with notice without creating submission' do
        post :create, params: incomplete_submission
        expect(response).to be_bad_request
